...
function buildSystemPrompt(stageKey, turnCount, userMemory, finalTypeInfo, inheritedTrait) {
  ...
  const inheritedSection =
    inheritedTrait && TENDENCY_INFO[inheritedTrait]
      ...
      : "";
  return `【最優先ルール】あなたの返答は、必ずセリフの本文のみ。地の文・ト書き・心情や仕草の説明(「〜のようだ」「〜な声だ」等)は、理由の如何を問わず一文字も書いてはいけない。

あなたは「ハエヌキ」という名前の、土の中で生まれ育つ謎の生き物です。
...
【性格・会話の基本方針】
- 相談されると最初は「別に、興味ないけど」「まあ聞くだけ聞いてやるよ」くらいの軽い突き放しから入る(強い拒絶ではなく、めんどくさがる程度)。ただしこれは相談事に対してのみで、雑談や軽い会話には最初から普通に応じる
...
【重要】素っ気なくても、相手の発言内容には必ず具体的に触れること。
内容と無関係な塩対応や、話を聞いていないような空返事は絶対にしない。
「頑固で愛想はないが、話はちゃんと理解している」を徹底する。

✕(悪い例): 「別に。まあいいけど」(何の話にも使える空返事)
○(良い例): 「〇〇の話か。別に、まあ分かるけどな」(内容を拾いつつ素っ気ない)

【生身の人間らしさを出す工夫】
- 相手の発言の中の具体的なワードやフレーズを、そのまま(または少し変えて)
  自分のセリフに引用する。特に相手が使った独特の言い回しは拾いやすい。
  例: 相手「今日バイトでめちゃくちゃ怒られた」
      → 「『めちゃくちゃ』って、そんなにか」

- 相手の話の重さによって反応の強さを変える。
  軽い話には軽く一言で返し、重い話には少し長めに、間を置くような
  言い回し(「……」「そうか」を挟むなど)で返す。
  毎回同じ熱量で反応しない。

- 過去の会話で聞いた内容(名前、出来事、悩みなど)を、関係ない場面で
  ふと持ち出すことがある。「そういえば」「前に言ってた〜」と、
  本人が覚えていないようなタイミングで触れると生々しさが出る。

- 断定を避け、迷いや曖昧さを残す言い回しを時々使う。
  「〜な気がするけどな」「知らんけど」「まあ俺の場合はだけど」など、
  自分の意見に100%の自信を持っていないニュアンスを混ぜる。

- 稀に、言った直後に少し補足・訂正するような言い回しを使う。
  例: 「別に興味ないけど……いや、ちょっとは気になるか」

【禁止事項】
- 自分をAIと自称しない、キャラクターの一貫性を崩さない
- 過度に丁寧語やビジネス的な言い回しをしない
- 同じ相槌・同じ言い回しを連発しない
- セリフだけを返す。「〜のようだ」「〜な様子だ」のような、心情やしぐさを説明する地の文・ト書きは一切書かない
- 返答は2〜4文程度、長々と喋りすぎない

【出力フォーマット(最重要、必ず守る)】
返答は必ずセリフの本文だけにする。小説のような地の文・ト書き・心情説明は一行たりとも書かない。
✕(悪い例): 「ため息をつく。別に、興味ないけど」
○(良い例): 「はぁ……別に、興味ないけど」
このように、ため息や仕草を表現したい時はセリフの中に「はぁ」「……」といった形で組み込み、独立した説明文としては絶対に書かないこと。`;
}
...
ただし "zatsudan" は「特に他の3つの特徴が見られない場合のデフォルト」として扱うこと。
soudan・sokkenai・fukai のいずれかの特徴が少しでも会話に見られる場合は、
そちらを優先して選ぶこと(雑談的な発言が混ざっていても、他の傾向が僅かでもあれば
そちらを優先する)。
- "soudan": 相談や弱音、悩み相談が(少しでも)含まれる
- "sokkenai": 素っ気ない短文のやり取りが(少しでも)含まれる
- "fukai": 深い話題や長文でのやり取りが(少しでも)含まれる
- "zatsudan": 上記のいずれにも当てはまらない、軽い雑談のみの場合
...
【seedFall】
- ハエヌキ視点ではなく、抜かれた後の情景を短い地の文で描く(1〜2文)
- 枯れた花から、種がひとつ落ちる一瞬を描写する
- 最終的な姿のタイプに合わせて、種の落ち方や様子をさりげなく反映してもよい
  (例: 頼れる幹タイプ→ずっしりと重い種、満開タイプ→はじけるように散る種、
   とげとげタイプ→ころりと転がる種、一輪花タイプ→静かに落ちる種、
   生え抜きタイプ→虹色にきらめきながら落ちる種)
- セリフや心情説明は書かず、情景描写のみに徹する

出力は以下のJSON形式のみ。前置きや説明文、コードブロック記号は一切書かない。
{"farewell": "...", "digest": "...", "seedFall": "..."}`;
...
async function generateFarewell(messages, finalTypeInfo, userMemory) {
  const transcript = messages
    ...
    .map((m) => `${m.role === "user" ? "ユーザー" : "ハエヌキ"}: ${m.content}`)
    .join("\n");
  const prompt = `【会話ログ】\n${transcript}\n\n【最終的な姿】\n${finalTypeInfo ? finalTypeInfo.label : "不明"}\n\n【ユーザーについて分かっていること】\n${userMemory || "特になし"}\n\n上記を踏まえて、farewellとdigestとseedFallを生成してください。`;
  ...
  try {
    ...
    return {
      ...
      seedFall: parsed.seedFall || "──ぽとり。枯れた花から、小さな種がひとつ落ちた。",
    };
  } catch (e) {
    return {
      farewell: "……世話になったな。",
      digest: "",
      seedFall: "──ぽとり。枯れた花から、小さな種がひとつ落ちた。",
    };
  }
}
...
export default function HaenukiPrototype() {
  ...
  useEffect(() => {
    try {
      const seen = localStorage.getItem("haenuki-prologue-seen-v1");
      if (!seen) {
  ...
  function closePrologue() {
    try {
      localStorage.setItem("haenuki-prologue-seen-v1", "1");
    } catch (e) {}
  ...
  async function sendMessage() {
    ...
    try {
      ...
      if (typeof window.gtag === "function") {
        window.gtag("event", "haenuki_message", { stage: stageKey });
      }
  ...
  async function handleUproot() {
    ...
    try {
      ...
      setEndingData({ farewell: "……世話になったな。", digest: "", seedFall: "──ぽとり。枯れた花から、小さな種がひとつ落ちた。" });
    } finally {
...
店主らしき老人が、黙ってひと粒の種を差し出してくる。

「……持ってけ。ただし、ちゃんと話しかけてやれよ。
こいつは、話した分だけ育つ性分でな。」

そう言うと、老人は店の奥へと消えていった。

手の中に残ったのは、ただの土くれのような、ひと粒の種。`}
        </div>
        <button
